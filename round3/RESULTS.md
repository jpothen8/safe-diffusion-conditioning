...
See [METHODS_EXPLAINED.md](METHODS_EXPLAINED.md) and the pinned [implementation](src/bayesfp.py). No official BayesFP repository was located in the paper links or recorded repository search. These are our controlled experiments using a paper-derived implementation, not reproduced author benchmark scores.
...
The additional [adapter audit](results/adapter_checks.json) confirms bitwise zero-guidance equivalence for both new MuJoCo models after applying the identical physical postprocessing already used in evaluation. An initial check omitted that final clip and detected a 1.4e-5 DDPM scheduler overshoot; its log is retained. Applying the actual policy's documented clipping resolves the mismatch without changing any experiment. Both safety-cost gradients agree with autograd within 7.5e-9, and uniform/one-hot resampling checks pass. The [reproduction audit](results/reproduction_audit.json) checks all arrays in 17 HalfCheetah NPZ files and the full snapshots for exact equality.

The dependency overlay uses Gymnasium 1.2.0, MuJoCo 3.3.5, SB3 2.7.0 and the preserved parent PyTorch 2.7.1+cu128 stack; the inspected card is compute capability 12.0 and driver 595.84. See [requirements.lock.txt](requirements.lock.txt), [parent host record](../records/host.json), and [README.md](README.md). Numerical outcomes of the fresh-directory HalfCheetah command reproduction match the original; it is not a new independent sample. No physical hardware was operated and no results were published.

The draft-specific deliverables are [PAPER_INTEGRATION.md](PAPER_INTEGRATION.md), [paper_experiments.tex](paper_experiments.tex) and automatically generated [paper_results.tex](paper_results.tex). They separate the supported empirical claims from the draft's unresolved theoretical proofs.
