...
def action_mmd(first, second, bandwidth=.2):
    ...
    def kernel(x, y):
        squared = ((x*x).sum(1)[:, None] + (y*y).sum(1)[None] - 2*x@y.T) / x.shape[1]
        return np.exp(-np.maximum(squared, 0) / (2 * bandwidth**2))

    return float(np.sqrt(max(0, kernel(first, first).mean() + kernel(second, second).mean()
...
def crossed_intervals(differences, seed, draws=20000):
    ...
    if not np.isfinite(values).all():
        return {"complete": False, "reason": "At least one requested contrast is missing."}
    ...
    distribution = values[model_indices[:, :, None], state_indices[:, None, :]].mean((1, 2))
    model_means = values.mean(1)
    mean = float(model_means.mean())
    halfwidth = float(student_t.ppf(.975, models - 1) * model_means.std(ddof=1) / np.sqrt(models))
    return {
        "complete": True, "mean": mean, "crossed_bootstrap_ci95": np.quantile(distribution, [.025, .975]).tolist(),
        "model_t_ci95": [mean - halfwidth, mean + halfwidth],
        "model_means": model_means.tolist(), "positive_models": int((model_means > 0).sum()),
        "models": models, "states": states, "replicates": differences.shape[-1],
    }
