...
CONTROL_PERIOD = {"Walker2d": 0.008, "HalfCheetah": 0.05}


def load_expert(task, level="expert"):
    return SAC.load(
        expert_path(task, level),
        device="cpu",
    ...
    try:
        ...
        for _ in range(round(warm_seconds / CONTROL_PERIOD[task])):
            ...
            if terminated or truncated:
                raise RuntimeError(
                    "Snapshot warmup terminated; do not replace its seed."
                )
...
def restore(saved):
    ...
    mujoco.mj_setState(
        env.unwrapped.model,
        env.unwrapped.data,
        np.array(saved["integration_state"]),
        saved["state_spec"],
    )
...
def rollout(task, actor, snapshots, chunks=None, seconds=4.0):
    ...
    if chunks is not None and (
        chunks.shape != (count, horizon, 6) or horizon > controls
    ):
        raise ValueError(
            "Chunks must fit the evaluation interval and six-action environment."
        )
    ...
    try:
        for step in range(controls):
            states.append(
                np.array(
                    [
                        np.r_[env.unwrapped.data.qpos, env.unwrapped.data.qvel]
                        for env in envs
                    ]
                )
            )
            ...
            for index in np.flatnonzero(active):
                observations[index], rewards[index, step], term, trunc, _ = envs[
                    index
                ].step(action[index])
                lengths[index] += 1
                if term or trunc:
