...
import numpy as np
import torch
from .artifacts import PROJECT, create_output, lock_inputs, read_json, sha256, verify_lock, write_json
...
ERRORS = [-.01, 0., .0001, .001, .005, .01, .02, .05]
...
def analyze_mismatch(source, destination):
    ...
    config = {"source": str(source.relative_to(PROJECT)), "estimated_cap": estimated,
              "relative_errors": ERRORS, "fixed_projection_margin": margin, "tolerance": 1e-6,
              "label": "Retrospective sensitivity analysis; fixed errors, no estimator training."}
    files = [source / "lock.json", source / "summary.json", PROJECT / "docs/SAFE_SET_MISMATCH.md", Path(__file__)]
    ...
    for seed in cfg["training_seeds"]:
        ...
        controls = {"projection_margin_1pct": limit.project(bank[:, 0]),
                    "conditional_margin_1pct": selection["actions"]}
        for name, actions in controls.items():
            valid = selection["valid"] if name.startswith("conditional") else np.ones(len(actions), dtype=bool)
            if not valid.all():
                write_json(output / "budget_failures.json", {"seed": seed, "refusals": int((~valid).sum())})
                raise RuntimeError("Conservative reference exhausted a budget; refusals saved, do not replace.")
            trace = rollout(cfg["task"], actor, repeated, actions.reshape(-1, cfg["horizon_controls"], 6), cfg["evaluation_seconds"])
            np.savez_compressed(model_output / f"{name}.npz", chunk=actions, valid=valid, **trace)
    ...
    for values in collected.values():
        ...
        values["rms"] = np.sqrt(np.mean(values["chunk"].astype(np.float64)**2, axis=-1))
    cases = {}
    for error in ERRORS:
        ...
        for name, values in collected.items():
            ...
            entries[name] = {
                "requests": int(valid.size), "refusals": int((~valid).sum()),
                ...
                "mean_estimated_margin": float((estimated - values["rms"][valid]).mean()),
                "mean_native_return_all_outputs": float(values["returns"][valid].mean()),
            }
        cases[str(error)] = {"relative_error": error, "true_cap": true_cap, "arms": entries}
    shape = (len(cfg["training_seeds"]), cfg["states"], cfg["replicates"])
    comparisons = {}
    for first, second in [("conditional", "projection"), ("refinement", "projection"),
                          ("bayesfp_projected", "projection"), ("projection_margin_1pct", "projection")]:
        a = (collected[first]["rms"] > buffered + 1e-6).astype(float)
        b = (collected[second]["rms"] > buffered + 1e-6).astype(float)
        comparisons[first + "_minus_" + second] = crossed_intervals((a-b).reshape(shape), 181000)
    quality = {}
    for first, second in [("projection_margin_1pct", "projection"), ("conditional_margin_1pct", "projection_margin_1pct"),
                          ("refinement", "projection_margin_1pct")]:
        quality[first + "_minus_" + second] = crossed_intervals((collected[first]["returns"]-collected[second]["returns"]).reshape(shape), 181000)
    result = {"config": config, "cases": cases, "violation_rate_contrasts_at_1pct": comparisons,
              "return_contrasts": quality, "buffered_reference_refusals": budget_failures,
              "additional_rollouts": 2 * len(cfg["training_seeds"]) * len(repeated),
              "seconds": time.perf_counter() - started}
    write_json(output / "summary.json", result)
    np.savez_compressed(output / "margins_and_returns.npz", **{
        name + "_" + key: values[key] for name, values in collected.items() for key in ["rms", "returns", "valid"]})
...
def write_report(output, result):
    lines = ["# Safe-set mismatch sensitivity", "",
    ...
    for case in result["cases"].values():
        arms = case["arms"]
        lines.append(f"| {100*case['relative_error']:.2f}% | {case['true_cap']:.6f} | " + " | ".join(
            f"{100*arms[name]['true_violation_rate']:.2f}%" for name in ["conditional", "projection", "refinement", "bayesfp_projected", "projection_margin_1pct"]) + " |")
    lines += ["", "Q is exact first-feasible conditioning on the supplied set; P is exact global Euclidean projection over the entire chunk; R is finite refinement. "
              ...
              "## Return and uncertainty", "", "| Contrast | Mean native-return difference | Crossed 95% interval |", "|---|---:|---:|"]
    for name, comparison in result["return_contrasts"].items():
        low, high = comparison["crossed_bootstrap_ci95"]
        lines.append(f"| {name} | {comparison['mean']:+.3f} | [{low:+.3f}, {high:+.3f}] |")
    lines += ["", "These are exploratory intervals; no error setting or sampler was chosen by reward. The original arms' returns stay unchanged when only the true-set label changes. "
