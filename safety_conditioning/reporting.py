...
def build_report(output):
    ...
    for index, model in enumerate(summary["models"]):
        row = {"seed": model["training_seed"], "base_return": model["base_mean_return"], "acceptance": model["acceptance"]}
        for arm in names:
    ...
    totals = {arm: {
        ...
        "native_failures": sum(m["arms"][arm]["native_failures"] for m in summary["models"]),
        "nonpositive_returns": sum(m["arms"][arm]["nonpositive_returns"] for m in summary["models"]),
        "mean_return": float(np.mean([r[arm] for r in rows])),
        "mean_boundary_fraction": float(np.mean([m["arms"][arm]["boundary_fraction"] for m in summary["models"]])),
        "mean_mmd_to_conditional": float(np.mean([m["arms"][arm]["mmd_to_conditional"] for m in summary["models"]])),
    } for arm in names}
    write_json(output / "totals.json", totals)
    decision = ("The preregistered useful-advantage criterion was met." if summary["useful_advantage_replicated"]
    ...
    for row in rows:
        values = [row[k] for k in ["base_return", "conditional", "projection", "refinement", "bayesfp_projected", "refinement_minus_bayesfp_projected"]]
        lines.append(f"| {row['seed']} | " + " | ".join(f"{v:.2f}" for v in values) + " |")
    lines += ["", "Q is first-feasible rejection; P is exact global full-chunk projection; R is the fixed finite "
              ...
              "", "## Secondary descriptive comparisons", "", "| Contrast | Mean [crossed 95% interval] | Model t 95% interval |",
              "|---|---:|---:|"]
    for key, label in [("conditional_minus_projection", "Conditioning − projection"), ("refinement_minus_projection", "Refinement − projection")]:
        lines.append(f"| {label} | {interval(comparisons[key])} | {interval(comparisons[key], 'model_t_ci95')} |")
    lines += ["", "These secondary intervals are descriptive, without a new family of superiority claims. "
              "A refinement gain over BayesFP does not establish accurate conditional sampling or a conditioning-target advantage.",
              "", "## All requested outcomes", "", "| Arm | Mean return | Unsafe/requested | Native terminations | Nonpositive-return rollouts | Boundary fraction | MMD to Q |",
              "|---|---:|---:|---:|---:|---:|---:|"]
    for arm, entry in totals.items():
        lines.append(f"| {arm} | {entry['mean_return']:.2f} | {entry['unsafe']}/{total} | {entry['native_failures']} | "
    ...
    lines += ["", f"Rejection refusals: **{summary['refusals']}**. Hard-output violations: **{summary['hard_output_violations']}**. "
              ...
              "", "## Cost and provenance", "", "| Item | Mean seconds/model |", "|---|---:|"]
    for name in ["base_proposal_bank", "projection", "refinement", "bayesfp"]:
        lines.append(f"| {name} | {np.mean([m['timing_seconds'][name] for m in summary['models']]):.3f} |")
    lines += ["", "Each bank contains 32,768 proposals (128/request), all saved. Logical first-accept costs are saved separately. "
              ...
              "", "[Locked protocol](../../docs/HALFCHEETAH_REPLICATION.md) · [Config/code/asset hashes](lock.json) · "
    ...
    for ax, key, title in zip(axes, comparisons, ["Refinement − (BayesFP + projection)", "Conditioning − projection", "Refinement − projection"]):
        ...
        ax.errorbar(result["mean"], len(rows) + .5, xerr=[[result["mean"] - lower], [upper - result["mean"]]],
                    fmt="D", color="#b35806", capsize=4)
        ax.axvline(0, color=".4", linewidth=1)
        if key == "refinement_minus_bayesfp_projected":
            ax.axvline(config["minimum_useful_return_gain"], color="#b35806", linestyle="--", label="Fixed useful-effect threshold")
        ax.set_yticks(list(range(len(rows))) + [len(rows) + .5], [str(r["seed"]) for r in rows] + ["Aggregate 95% CI"])
        ax.invert_yaxis()
    ...
    tex = [r"\subsection{Independent-model replication}",
           ...
           f"The mean favored refinement in {primary['positive_models']} of eight models. " + decision + " "
