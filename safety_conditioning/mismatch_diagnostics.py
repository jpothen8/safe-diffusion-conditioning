...
LABELS = {"conditional": "Conditioning Q", "projection": "Exact projection P",
          "refinement": "Finite refinement R", "bayesfp": "Raw BayesFP",
...
def saved_arrays(output):
    ...
    for arm in (*ARMS, *CONTROLS):
        ...
        for seed in cfg["training_seeds"]:
            root = output / f"seed_{seed}" if arm in CONTROLS else source / f"seed_{seed}/evaluation"
            with np.load(root / f"{arm}.npz") as saved:
                arrays["actions"].append(saved["chunk"])
                arrays["rms"].append(np.linalg.norm(saved["chunk"].astype(float), axis=1) / np.sqrt(saved["chunk"].shape[1]))
...
def audit_mismatch(output):
    ...
    cap = result["config"]["estimated_cap"] * (1 - result["config"]["fixed_projection_margin"])
    ...
    for arm, values in arrays.items():
        ...
        for key in ("rms", "returns", "valid"):
            np.testing.assert_allclose(values[key], saved_margins[f"{arm}_{key}"], atol=1e-14, rtol=0)
        for case in result["cases"].values():
            ...
            np.testing.assert_allclose(entry["rate_by_model"], violations.mean(1), atol=0, rtol=0)
            for name, metric in [("mean_excess_all_outputs", excess.mean()),
    ...
    for index, seed in enumerate(cfg["training_seeds"]):
        ...
        with np.load(folder / "proposals.npz") as data:
            bank = data["actions"].reshape(expected_shape[1], cfg["proposal_budget"], -1)
        ...
        assert np.array_equal(arrays["conditional_margin_1pct"]["actions"][index], selected)
        ...
        np.testing.assert_allclose(arrays["projection_margin_1pct"]["actions"][index], independent_projection, atol=2e-7, rtol=0)
        selection_costs.append({"seed": seed, "bank_acceptance": float(feasible.mean()),
                                "logical_proposals_to_first_accept": int((first + 1).sum()),
                                "stored_proposals": int(feasible.size), "refusals": 0})
        for arm in CONTROLS:
            with np.load(output / f"seed_{seed}/{arm}.npz") as saved:
                assert not saved["terminated"].any()
                assert np.array_equal(saved["actions"][:, :cfg["horizon_controls"]].reshape(len(bank), -1), saved["chunk"])
                assert saved["executed"].all()
                replay = rollout(cfg["task"], actor, [snapshots[i // cfg["replicates"]] for i in replay_indices],
                                 saved["actions"][replay_indices], cfg["evaluation_seconds"])
                error = float(np.max(np.abs(replay["rewards"] - saved["rewards"][replay_indices])))
                assert error == 0
    ...
    for group in ("return_contrasts", "violation_rate_contrasts_at_1pct"):
        for name, entry in result[group].items():
            ...
            if group == "return_contrasts":
                ...
                difference = (arrays[first]["rms"] > cap + 1e-6).astype(float) - (arrays[second]["rms"] > cap + 1e-6)
            values = difference.reshape(models, states, cfg["replicates"]).mean(-1)
            boot = np.einsum("bi,ij,bj->b", mw, values, sw)
            np.testing.assert_allclose(entry["crossed_bootstrap_ci95"], np.quantile(boot, [.025, .975]), atol=1e-10, rtol=0)
            np.testing.assert_allclose(entry["model_means"], values.mean(1), atol=1e-10, rtol=0)
    report = {"passed": True, "analysis_locked_inputs_checked": inputs,
              ...
              "new_control_rollouts": result["additional_rollouts"], "selection_costs": selection_costs,
              ...
              "poor_return_counts": {arm: int((values["returns"] <= 0).sum()) for arm, values in arrays.items()},
              "source_snapshot_sha256": sha256(source / "snapshots.json")}
    write_json(output / "audit.json", report)
    print(f"Mismatch audit passed: {inputs} input hashes, all selections/projections/metrics, "
          f"{report['native_replayed_trajectories']} native replays with zero error.", flush=True)
...
def report_mismatch(output):
    ...
    report_path.write_text(report_path.read_text().replace("Estimated-cap overstatement", "True-cap reduction"))
    rows = []
    for case in result["cases"].values():
        for arm, entry in case["arms"].items():
            rows.append({"relative_cap_error": case["relative_error"], "true_cap": case["true_cap"],
                         "arm": arm, **{k: v for k, v in entry.items() if k != "rate_by_model"}})
    ...
    colors = {"conditional": "#0072B2", "projection": "#D55E00", "refinement": "#009E73",
              "bayesfp_projected": "#CC79A7", "projection_margin_1pct": "#333333",
              "conditional_margin_1pct": "#56B4E9"}
    for arm, color in colors.items():
        entries = [case["arms"][arm] for case in cases]
        style = "--" if arm in CONTROLS else "-"
        axes[0].plot(x, [100 * entry["true_violation_rate"] for entry in entries], style,
                     marker="o", markersize=3, color=color, label=LABELS[arm])
        axes[1].plot(x, [entry["mean_excess_all_outputs"] for entry in entries], style,
                     marker="o", markersize=3, color=color)
    for axis in axes:
        axis.set_xscale("symlog", linthresh=.01)
        axis.set_xticks([0, .01, .1, .5, 1, 2, 5], ["0", ".01", ".1", ".5", "1", "2", "5"])
    ...
    fig.suptitle("HalfCheetah: 8 frozen policies, 2,048 outputs per arm; retrospective cap-bias probe", fontsize=11)
    ...
    plt.close(fig)
    lines = ["", "## Violation uncertainty and magnitude", "",
    ...
    for name, entry in result["violation_rate_contrasts_at_1pct"].items():
        ...
        lines.append(f"| {LABELS[first]} minus {LABELS[second]} | {100*entry['mean']:+.2f} | [{low:+.2f}, {high:+.2f}] |")
    lines += ["", "All violations count here, but their size matters. At a 1% cap error, mean positive excess "
    ...
    with (output / "RESULTS.md").open("a") as stream:
        stream.write("\n".join(lines) + "\n")
    (output / "paper_mismatch.tex").write_text(r"""\paragraph{Exploratory sensitivity to safe-set error.}
