...
def train_policy(
    dataset_path, task, seed, output, *, horizon=8, steps=48000, device="cuda"
):
    output.mkdir(parents=True, exist_ok=False)
    dataset = EpisodeDataset(dataset_path)
    train_obs, train_actions, early = dataset.windows(
        "train", horizon, CONTROL_PERIOD[task]
    )
    val_obs, val_actions, _ = dataset.windows(
        "validation", horizon, CONTROL_PERIOD[task]
    )
    seed_torch(seed)
    ...
    train_obs, train_actions, val_obs, val_actions = [
        ...
        for value in (train_obs, train_actions, val_obs, val_actions)
    ]
    ...
    for step in range(steps):
        ...
        loss = (
            (
                model(
                    schedule.add_noise(train_actions[indices], noise, timestep),
                    timestep,
                    train_obs[indices],
                )
                - noise
        ...
        if (step + 1) % 8000 == 0:
            with torch.no_grad():
                ...
                val_loss = (
                    (
                        ema(
                            schedule.add_noise(val_actions[indices], noise, timestep),
                            timestep,
                            val_obs[indices],
                        )
                        - noise
            ...
            logs.append(row)
            print(
                f"train seed={seed} step={step + 1} validation={float(val_loss):.4f}",
                flush=True,
            )
    ...
    torch.save(
        {
            "dimension": horizon * 6,
            "state_dict": {key: value.cpu() for key, value in ema.state_dict().items()},
        },
        checkpoint,
    )
    summary = {
        ...
        "steps": steps,
        "batch": 512,
        "horizon_controls": horizon,
        "horizon_seconds": horizon * CONTROL_PERIOD[task],
        "dataset_sha256": sha256(dataset_path),
        "checkpoint_sha256": sha256(checkpoint),
        "seconds": time.perf_counter() - started,
        "selection": "Final fixed-step EMA; no selection by validation, reward or safety.",
        "logs": logs,
    }
