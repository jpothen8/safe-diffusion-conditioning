...
def refine(model, observations, raw, constraint, seed):
    ...
    actions = constraint.project_torch(
    ...
    with torch.inference_mode():
        for level, steps in [(20, 256), (5, 256), (0, 512)]:
            ...
            for step in range(steps):
                rate = 0.15 * sigma.square() / (1 + step / 64) ** 0.6
                score = -model(alpha_bar.sqrt() * actions, level, observations) / sigma
                proposal = (
                    actions
                    + rate * score
                    + (2 * rate).sqrt() * torch.randn_like(actions)
                )
...
def systematic_resample(weights):
    ...
    cumulative[:, -1] = 1
    positions = (
        torch.rand((batch, 1), device=weights.device)
        + torch.arange(particles, device=weights.device)
    ) / particles
    return torch.searchsorted(
        cumulative.contiguous(), positions.contiguous()
    ).clamp_max(particles - 1)
...
def bayesfp(
    ...
    *,
    ...
    effective_sizes = []
    ancestors = (
        torch.arange(particles, device=device)[None].expand(batch, particles).clone()
    )
    with torch.inference_mode():
    ...
    diagnostics = {
        ...
        "ess_mean": float(np.mean(effective_sizes))
        if effective_sizes
        else float(particles),
        "ess_min": float(np.min(effective_sizes))
        if effective_sizes
        else float(particles),
        "unique_initial_ancestors_mean": float(
            np.mean([len(torch.unique(row)) for row in ancestors])
        ),
    }
