...
class EpisodeDataset:
    def __init__(self, path):
        ...
        splits = [set(self.data[name + "_groups"]) for name in ("train", "validation", "test")]
    ...
    def windows(self, split, horizon, control_period):
        ...
        for start, length, group in zip(data["episode_starts"], data["episode_lengths"], data["groupids"]):
            ...
            windows = np.lib.stride_tricks.sliding_window_view(episode, horizon, axis=0).transpose(0, 2, 1)
        ...
        return np.concatenate(observations), np.concatenate(actions), np.concatenate(early)
