...
import numpy as np
import torch
from .artifacts import read_json, sha256, verify_lock, write_json, recorded_path
...
def audit_study(output):
    ...
    mean, std = EpisodeDataset(recorded_path(config["dataset"])).normalizer()
    ...
    for seed in config["training_seeds"]:
        ...
        assert (
            digest
            == read_json(model_root / "training/summary.json")["checkpoint_sha256"]
        )
        ...
        with np.load(evaluation / "selection.npz") as data:
            ...
            assert np.array_equal(
                data["actions"][valid], bank[np.arange(len(bank))[valid], first[valid]]
            )
        ...
        for arm in ARMS:
            with np.load(evaluation / f"{arm}.npz") as data:
                ...
                safe = (
                    np.sqrt(np.mean(actions[good] ** 2, axis=-1))
                    <= config["rms_limit"] + 1e-6
                )
                ...
                assert np.isclose(
                    returns[arm][good].mean(),
                    entry["mean_return_given_output"],
                    atol=1e-10,
                    rtol=0,
                )
                ...
                if good.any():
                    ...
                    assert np.array_equal(
                        data["actions"][good, : config["horizon_controls"]].reshape(
                            good.sum(), -1
                        ),
                        actions[good],
                    )
                ...
                if arm == "conditional":
                    assert np.array_equal(
                        actions[good], bank[np.arange(len(bank))[good], first[good]]
                    )
        all_deltas.append(
            (returns["refinement"] - returns["bayesfp_projected"]).reshape(count, reps)
        )
        details.append(
            {"seed": seed, "checkpoint_sha256": digest, "refusals": int((~valid).sum())}
        )
    ...
    assert np.allclose(
        ...
        primary["crossed_bootstrap_ci95"],
        atol=1e-10,
        rtol=0,
    )
    ...
    for arm in ARMS:
        with np.load(folder / f"{arm}.npz") as data:
            ...
            replay_errors[arm] = float(
                np.max(np.abs(replay["rewards"] - data["rewards"][:1]))
            )
            assert replay_errors[arm] == 0
            assert np.array_equal(replay["states"], data["states"][:1])
    report = {
        "passed": True,
        "locked_input_hashes_checked": checked_inputs,
        "independent_checkpoints": len(set(checkpoints)),
        "models": details,
        "all_selections_and_global_projections_verified": True,
        "train_only_normalizers_exact": True,
        "crossed_bootstrap_independently_verified": True,
