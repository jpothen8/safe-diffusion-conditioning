...
def collect_episodes(task, output):
    ...
    np.savez_compressed(
        ...
        episode_starts=np.r_[0, ends[:-1]],
        episode_lengths=lengths,
        groupids=groups,
        levels=levels,
        terminated=terminals,
        train_groups=permutation[:48],
        validation_groups=permutation[48:56],
        test_groups=permutation[56:],
    )
    summary = {
        "task": task,
        "episodes": len(lengths),
        "controls": sum(lengths),
        "collection_seed_start": 92000,
        "split_seed": 92100,
