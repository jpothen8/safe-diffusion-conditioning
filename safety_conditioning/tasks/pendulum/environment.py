...
def actor():
    ...
    with zipfile.ZipFile(PROJECT / "assets/td3-Pendulum-v1.zip") as archive:
        weights = torch.load(
    ...
    model.load_state_dict(
        {
            k.removeprefix("actor.mu."): v
            for k, v in weights.items()
            if k.startswith("actor.mu.")
        }
    )
    model.eval()
...
def observation(state):
    return np.stack(
        [np.cos(state[..., 0]), np.sin(state[..., 0]), state[..., 1]], axis=-1
    ).astype(np.float32)
...
def dynamics(state, actions):
    ...
    reward = -(wrapped**2 + 0.1 * velocity**2 + 0.001 * u**2)
...
def rollout(model, states, first_actions=None, reflection=False, steps=200):
    ...
    for t in range(steps):
        ...
        aa = (
            first_actions[:, t]
            if first_actions is not None and t < first_actions.shape[1]
            else act(model, obs, reflection)
        )
        history.append(state.copy())
        observations.append(obs)
        actions.append(aa)
        state, rr = dynamics(state, aa)
        rewards.append(rr)
    return {
        "states": np.stack(history, axis=1),
