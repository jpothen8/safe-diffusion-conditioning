...
def main():
    ...
    assert len(files) == len(cfg["evaluation_seeds"]), (
        "Incomplete experiment; do not summarize a partial run."
    )
    ...
    for cap in cfg["speed_caps"]:
        ...
        for seed in cfg["evaluation_seeds"]:
            ...
            diff = [
                r["conditional"]["mean_reward"] - r["projection"]["mean_reward"]
                for r in sr
            ...
            per_state.append(
                {
                    ...
                    "generated": n,
                    "acceptance": k / n,
                    "acceptance_CI95": interval(k, n),
                    "complete_pairs": len(diff),
                    "mean_difference": float(np.mean(diff)) if diff else None,
                }
            )
        ...
        ci = (
            np.quantile(
                state_differences[indices].mean(axis=1), [0.025, 0.975]
            ).tolist()
            if complete
            else None
        )
        res = {
            "requested_pairs": len(rr),
            "complete_pairs": len(differences),
            "paired_difference": float(np.mean(differences)) if differences else None,
            "paired_cluster_CI95": ci,
            "missing_outcome_bounds": [float(np.mean(lower)), float(np.mean(upper))],
        ...
        for method in ["conditional", "projection"]:
            ...
            res[method] = {
                "outputs": len(outputs),
                "unsafe": sum(o["unsafe_output"] for o in outputs),
                "mean_reward_outputs": float(
                    np.mean([o["mean_reward"] for o in outputs])
                )
                if outputs
                else None,
                "operational_score": float(
                    np.mean([r[method + "_operational_score"] for r in rr])
                ),
                "mean_max_reward": float(np.mean([o["max_reward"] for o in outputs]))
                if outputs
                else None,
                ...
                "mean_chunk_reward": float(
                    np.mean([o["chunk_mean_reward"] for o in outputs])
                )
                if outputs
                else None,
                "route_counts": dict(collections.Counter(o["route"] for o in outputs)),
                "rotation_counts": dict(
                    collections.Counter(o["rotation_mode"] for o in outputs)
                ),
                "mean_roughness": float(
                    np.mean([o["action_roughness"] for o in outputs])
                )
                if outputs
                else None,
            }
                        ...
                        modes.append(
                            {
                                ...
                                "n": len(values),
                                "mean_reward": float(
                                    np.mean([v["mean_reward"] for v in values])
                                ),
                                "mean_max_reward": float(
                                    np.mean([v["max_reward"] for v in values])
                                ),
                                "contact_chunks": sum(
                                    v["contact_controls"] > 0 for v in values
                                ),
                                "mean_block_displacement": float(
                                    np.mean([v["block_displacement"] for v in values])
                                ),
                            }
                        )
    ...
    for ax, cap in zip(axs, cfg["speed_caps"]):
        for method, color in [("conditional", "#2878b5"), ("projection", "#d45b24")]:
            xx = np.sort(
                [
                    r[method]["actual_chunk_max_speed"]
                    for r in rows
                    if r["cap"] == cap and r[method] is not None
                ]
            )
            ax.step(
                xx,
                np.arange(1, len(xx) + 1) / len(xx),
                where="post",
                label=method,
                color=color,
            )
        ax.axvline(cap, color="black", ls="--", lw=0.8)
        ax.set(
            title=f"Speed cap {cap} px/s",
            xlabel="Maximum pusher speed (px/s)",
        ...
        for method, color in [("conditional", "#2878b5"), ("projection", "#d45b24")]:
            for rep in range(cfg["replicates"]):
                ...
                if key in archive:
                    ...
                    ax.plot(trace[:, 0], trace[:, 1], color=color, alpha=0.5, lw=0.8)
    ...
    fig.suptitle(
        "0.8 s pusher paths at the same snapshots: blue conditional, orange projected; green × block origin"
    )
    ...
    for j, cap in enumerate(cfg["speed_caps"]):
        values = [
            r["mean_difference"]
            for r in per_state
            if r["cap"] == cap and r["mean_difference"] is not None
        ]
        ax.scatter(np.full(len(values), j), values, alpha=0.6)
        ...
        if res["paired_cluster_CI95"]:
            ...
            ax.errorbar(
                ...
                yerr=[[res["paired_difference"] - lo], [hi - res["paired_difference"]]],
                fmt="ks",
                capsize=4,
            )
    ax.axhline(0, color="black", lw=0.7)
    ax.axhline(0.05, color="green", ls="--", label="Minimum useful improvement")
    ax.set(
        xticks=range(3),
        xticklabels=cfg["speed_caps"],
        xlabel="Speed cap (px/s)",
        ylabel="Conditional − projection mean reward",
    )
    ax.legend()
