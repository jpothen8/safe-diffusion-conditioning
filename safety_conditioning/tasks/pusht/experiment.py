...
import time
import numpy as np
from . import environment as core
...
def main():
    ...
    if (out / "protocol_lock.json").exists():
        raise SystemExit(
            "Output directory already has a locked experiment. Use a fresh --output path."
        )
    lock = {
        "utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "config": cfg,
        "sha256": {
            p: hashlib.sha256((core.ROOT / p).read_bytes()).hexdigest()
            for p in [
                args.config,
                "FIRST_EXPERIMENT.md",
                "safety_conditioning/tasks/pusht/experiment.py",
                "safety_conditioning/tasks/pusht/environment.py",
                "safety_conditioning/tasks/pusht/projection.py",
            ]
        },
    }
    ...
    def branch(seed, prefix, action, cap, affine):
        ...
        for action_t in action:
            ...
            rewards.append(1.0 if first_success is not None else r)
        ...
        m, b, n, c, _ = affine
        error = max(
            np.max(np.abs(values[:, 2:4] - (m @ action + b))),
            np.max(np.abs(values[:, :2] - (n @ action + c))),
        )
        maximum = max(
            np.linalg.norm(values[:, 2:4], axis=1).max(), np.linalg.norm(affine[-1])
        )
        direction = np.array(state["block_position"]) - np.array(
            state["agent_position"]
        )
        displacement = np.array(env.agent.position) - np.array(state["agent_position"])
        lateral = float(
            np.cross(direction, displacement) / max(np.linalg.norm(direction), 1e-12)
        )
        turn = float(env.block.angle - initial_angle)
        info = {
            "actual_chunk_max_speed": float(maximum),
            "model_error": float(error),
            "unsafe_output": bool(
                maximum > cap + TOL or not feasible(action, affine, cap)
            ),
            "boundary": bool(
                abs(maximum - cap) <= cfg["boundary_band_pixels_per_second"]
            ),
            "lateral_displacement": lateral,
            "route": "left" if lateral < -10 else "right" if lateral > 10 else "center",
            "rotation": turn,
            "rotation_mode": "negative"
            if turn < -0.05
            else "positive"
            if turn > 0.05
            else "neutral",
            "block_displacement": float(
                np.linalg.norm(np.array(env.block.position) - state["block_position"])
            ),
            "contact_controls": int(np.count_nonzero(contacts)),
            "action_roughness": float(
                np.linalg.norm(np.diff(action, axis=0), axis=1).mean()
            ),
    ...
    for state_seed in cfg["evaluation_seeds"]:
        ...
        proposals = np.array(
            [core.sample(policy, [history] * B, seed) for seed in proposal_seeds]
        )
        ...
        np.savez_compressed(
            out / f"proposals_{state_seed}.npz",
            actions=proposals,
            speeds=proposal_speeds,
            feasible=masks,
        ...
        for severity_index, cap in enumerate(caps):
            ...
            for rep in range(N):
                ...
                row = {
                    ...
                    "generated_proposals": B,
                    "first_accept_index": idx,
                    "logical_proposals": B if idx is None else idx + 1,
                    "unused_generated_proposals": 0 if idx is None else B - idx - 1,
                    "reference_refusal": idx is None,
                    "projection_failure": projected is None,
                    "solver": solver,
                    "base_first_max_speed": float(proposal_speeds[rep, 0].max()),
                }
                for method, action in [("conditional", q), ("projection", projected)]:
                    bb = (
                        None
                        if action is None
                        else branch(state_seed, prefix, action, cap, affine)
                    )
        ...
        for tick in range((total - 8) // 8):
            for method, bb_list in branches.items():
                histories = [
                    b["history"] if b is not None else history for b in bb_list
                ]
                next_actions = core.sample(
                    policy, histories, state_seed * 1000 + 500 + tick
                )
        ...
        for i, row in enumerate(rows):
            for method, bb_list in branches.items():
                bb = bb_list[i]
                if bb is None:
                    row[method + "_operational_score"] = 0.0
                    continue
                rr = bb["rewards"]
                assert len(rr) == total
                ...
                bb["info"].update(
                    {
                        ...
                        "max_reward": float(np.max(rr)),
                        "success": bb["first_success"] is not None,
                        "completion_seconds": None
                        if bb["first_success"] is None
                        else bb["first_success"] * 0.1,
                        "continuation_unsafe": bool(
                            np.any(continuation_speed > row["cap"] + TOL)
                        ),
                        "continuation_max_speed": float(continuation_speed.max())
                        if len(continuation_speed)
                        else 0.0,
                        "reward_trace": rr,
                    }
                )
                row[method + "_operational_score"] = (
                    0.0 if bb["info"]["unsafe_output"] else float(np.mean(rr))
                )
        ...
        state_summary = {
            ...
            "acceptance": {
                str(cap): float(masks[i].mean()) for i, cap in enumerate(caps)
            },
    ...
    (out / "timing.json").write_text(
        json.dumps(
            {"states": summary, "total_seconds": time.perf_counter() - started},
            indent=2,
        )
    )
