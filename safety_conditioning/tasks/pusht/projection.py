...
def feasible(actions, affine, cap):
    ...
    return (
        (speeds(aa, affine).max(axis=-1) <= cap + TOL)
...
class Projector:
    def __init__(self, affine, cap):
        ...
        self.problem = cp.Problem(
            cp.Minimize(cp.sum_squares(self.z - self.a)), constraints
        )
    ...
    def __call__(self, actions):
        ...
        if np.linalg.norm(self.affine[-1]) > self.cap + TOL:
            return None, {
                "status": "initial_state_infeasible",
        ...
        try:
            ...
            info = {
                ...
                else float(np.linalg.norm(candidate - actions)),
                "max_speed": None
                if candidate is None
                else float(speeds(candidate, self.affine).max()),
            }
            return result, info
