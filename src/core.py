...
def load_policy(device="cuda"):
    ...
    payload = torch.load(ROOT / "assets/pusht_transformer.ckpt", map_location="cpu",
...
def sample(policy, histories, seed):
    ...
    with torch.inference_mode():
        result = policy.predict_action({"obs": torch.as_tensor(observations, device=policy.device)})
...
def numeric_state(env):
    return {"agent_position": list(env.agent.position), "agent_velocity": list(env.agent.velocity),
            "block_position": list(env.block.position), "block_velocity": list(env.block.velocity),
            "block_angle": env.block.angle, "block_angular_velocity": env.block.angular_velocity}
